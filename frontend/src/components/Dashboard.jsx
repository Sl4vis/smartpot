...
function PlantCard({ data, i }) {
  ...
  const statusBg = score >= 70 ? 'bg-green-50 dark:bg-green-950/40 text-green-700 dark:text-green-500' : score >= 40 ? 'bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400' : 'bg-red-50 dark:bg-red-950/20 text-red-600 dark:text-red-400';
  ...
  return (
    <Link to={`/plant/${plant.id}`} className={`card p-4 sm:p-5 block fade-in delay-${i + 1} group`}>
        ...
        <div className="flex items-start gap-3 min-w-0 flex-1">
          ...
          <div className="flex items-stretch gap-2.5 min-w-0">
            ...
            <div className="min-w-0 flex justify-center flex-col">
              <h3 className="font-semibold text-green-900 dark:text-green-100 truncate text-base sm:text-lg leading-tight">{plant.name}</h3>
              <p className="text-xs sm:text-sm text-sage-500 dark:text-green-700 truncate mt-1 leading-tight">{plant.species}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0">
          {unread_alerts > 0 && (
            <span className="w-5 h-5 rounded-full bg-red-100 dark:bg-red-900/20 text-red-600 dark:text-red-400 text-[10px] font-bold flex items-center justify-center">
              {unread_alerts}
            </span>
          )}
          <ChevronRight className="w-4 h-4 text-sage-300 dark:text-green-800 group-hover:text-green-500 dark:text-green-500 group-hover:translate-x-0.5 transition-all" />
        </div>
      </div>
      ...
      <div className="mt-3 sm:mt-4 pt-3 border-t border-sage-100 dark:border-green-900/20 flex items-center justify-between text-xs gap-3">
        <span className="truncate text-sage-400 dark:text-green-700">{plant.location}</span>
...
function StatusBadge({ status }) {
    ...
    return (
      <span className="inline-flex items-center gap-1 font-semibold text-green-600 dark:text-green-500 whitespace-nowrap">
        <span className="relative flex h-2 w-2">
          ...
          <span className="relative inline-flex rounded-full h-2 w-2 bg-green-500 dark:bg-green-500" />
        </span>
        Online
      </span>
    );
  ...
  return (
    <span className="inline-flex items-center gap-1 font-semibold text-red-500 dark:text-red-400 whitespace-nowrap">
      <span className="relative flex h-2 w-2">
...
function Metric({ icon: Icon, label, value, unit, color, warn }) {
  return (
    <div className={`flex items-center gap-2 sm:gap-2.5 px-2.5 sm:px-3 py-2 rounded-xl ${warn ? 'bg-red-50 dark:bg-red-950/20' : 'bg-sage-50 dark:bg-green-950/40'}`}>
      <Icon className="w-3.5 h-3.5 flex-shrink-0" style={{ color }} />
      <div className="min-w-0">
        <p className="text-[10px] text-sage-400 dark:text-green-700 leading-none mb-0.5">{label}</p>
        <p className={`text-sm font-semibold leading-none ${warn ? 'text-red-600 dark:text-red-400' : 'text-green-900 dark:text-green-100'}`}>
