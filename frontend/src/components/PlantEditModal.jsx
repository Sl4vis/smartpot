...
export default function PlantEditModal({ isOpen, onClose, plant, onSaved }) {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[70] flex items-end sm:items-center justify-center bg-sage-900/35 dark:bg-green-950/50 backdrop-blur-sm p-0 sm:p-4" onClick={onClose}>
      <div className="card w-full sm:max-w-2xl max-h-[92dvh] sm:max-h-[90vh] overflow-hidden rounded-t-3xl rounded-b-none sm:rounded-2xl shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between gap-4 px-4 sm:px-6 py-4 sm:py-5 border-b border-sage-100 dark:border-green-900/20 bg-white dark:bg-green-950/50">
          <div>
            <h2 className="text-xl font-bold text-green-900 dark:text-green-100">Upraviť rastlinu</h2>
            <p className="text-sm text-sage-500 dark:text-green-700">Predvyplnené údaje môžeš hneď upraviť.</p>
          </div>
          <button
            type="button"
            onClick={onClose}
            className="p-2 rounded-xl text-sage-400 dark:text-green-700 hover:text-green-700 dark:text-green-500 hover:bg-green-50 dark:hover:bg-green-900/30 dark:bg-green-950/40 transition-colors"
            title="Zavrieť"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="p-4 sm:p-6 space-y-4 sm:space-y-5">
            ...
            <div>
              ...
              {devicesLoading ? (
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    {hasAvailable ? (
                      <>
                        ...
                        <span className="text-green-600 dark:text-green-500">{devices.available.length} dostupných zariadení</span>
                      </>
                    ) : (
                      <>
                    ...
                    {devices.current_device && (
                      <span className="text-sage-400 dark:text-green-700">· aktuálne: {devices.current_device}</span>
                    )}
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  />
                  <div className="flex items-center gap-2 text-xs text-sage-400 dark:text-green-700">
            ...
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition-all bg-purple-50 dark:bg-purple-950/20 text-purple-600 dark:text-purple-400 hover:bg-purple-100 dark:bg-purple-900/20 disabled:opacity-40 disabled:cursor-not-allowed"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs text-sage-400 dark:text-green-700 mb-1">Max. svetlo (lux)</label>
                <input name="max_light" type="number" value={form.max_light} onChange={handleFieldChange} className="input-field" />
              </div>
            </div>

            <div>
              <label className="block text-xs text-sage-400 dark:text-green-700 mb-1">Obrázok URL (voliteľné)</label>
              <input name="image_url" value={form.image_url} onChange={handleFieldChange} placeholder="https://..." className="input-field" />
            </div>
          </div>

          <div className="px-4 sm:px-6 py-4 border-t border-sage-100 dark:border-green-900/20 bg-white dark:bg-green-950/50 flex flex-col-reverse sm:flex-row gap-3 sm:justify-end pb-[max(1rem,env(safe-area-inset-bottom))]">
